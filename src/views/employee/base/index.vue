        ...
        <div class="content" style="margin-top: 10px;">
          ...
          >
            ...
            <el-table-column
              ...
              align="center"
            ></el-table-column>
            <el-table-column prop="employeeNumber" label="员工编号" min-width="100" align="center" />
            <el-table-column prop="phone" label="手机号" min-width="100" align="center" />
            <!--<el-table-column prop="employeeAccount" label="登陆账号"/>-->
            <el-table-column prop="identityCard" label="身份证" min-width="100" align="center">
            ...
            <el-table-column prop="sex" label="性别" align="center">
              <template slot-scope="scope">
                <span
                  v-for="(item, index) in dictMap.em_sex"
                  v-show="item.value == scope.row.sex"
                  :key="index"
                >{{item.label}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="education" label="学历" align="center">
              <template slot-scope="scope">
                <span
                  v-for="(item, index) in dictMap.em_education"
                  v-show="item.value == scope.row.education"
                  :key="index"
                >{{item.label}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="jobTitle" label="职称" align="center">
              <template slot-scope="scope">
                <span
                  v-for="(item, index) in dictMap.em_job_title"
                  v-show="item.value == scope.row.jobTitle"
                  :key="index"
                >{{item.label}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="workStatus" label="员工状态" align="center">
              <template slot-scope="scope">
                <span
                  v-for="(item, index) in dictMap.em_work_status"
                  v-show="item.value == scope.row.workStatus"
                  :key="index"
                >{{item.label}}</span>
              </template>
            </el-table-column>
...
import { page, batchDel, downTemplate, exportExcel, infoById } from '@/api/employee/employee'
import { jobAll } from '@/api/employee/job'
  ...
  methods: {
    ...
    refreshPage () {
      ...
      page(this.pageParams).then(res => {
        if (res.code === 0) {
          res.data.records.forEach((p, i, arr) => {
            p.deptName = ''
            p.deptList.forEach((n) => {
              p.deptName += n.path + '、'
            })
          })
