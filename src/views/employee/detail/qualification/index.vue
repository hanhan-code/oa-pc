  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-upload
        ...
        :action="$network + '/emp/qualification/batch'"
        :limit="1"
        :file-list="fileList"
          ...
          <el-table-column prop="qualification" label="资质名称">
            <template slot-scope="scope">
              <span
                ...
                v-if="item.value == scope.row.qualification"
              >{{item.label}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="manageNo" label="管理号" />
          <el-table-column prop="qualificationCardNo" :show-overflow-tooltip="true" label="执业资格证编号"></el-table-column>
          <el-table-column prop="major" label="考试专业">
            <template slot-scope="scope">
              <span
                v-for="item in dictMap.em_register_major"
                v-if="item.value == scope.row.major"
              >{{item.label}}</span>
            </template>
          </el-table-column>

          <el-table-column prop="approvalDate" label="批准日期">
            <template slot-scope="scope">
              <span>{{$moment(scope.row.approvalDate).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="issueDate" label="批准日期">
            <template slot-scope="scope">
              <span>{{$moment(scope.row.issueDate).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { batchDel, page, exportExcel } from '@/api/employee/qualification'    // 执业资格
import initDict from '@/mixins/initDict'
import { downTemplate } from '@/api/employee'
import Form from './form'

export default {
  name: 'qualification',    // 执业资格
  mixins: [initDict],
  props: {
        ...
        if (this.selects.length > 0) {
          ...
          batchDel(ids).then(res => {
            if (res.code === 0) {
              ...
              this.$message({ message: '执业资格: '+res.msg + "；记录所对应证书记录已通过审核，您已无权进行更新操作", type: 'warning' })
            } else {
              this.$message({ message: res.msg, type: 'warning' })
            }
          })
