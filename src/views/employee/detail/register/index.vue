<template>
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-button type="primary" size="mini" @click="edit" :disabled="button.edit" plain>编辑</el-button>
      <el-button type="primary" size="mini" @click="look" :disabled="button.edit" plain>查看</el-button>
      <el-button type="danger" size="mini" @click="del" :disabled="button.delete" plain>删除</el-button>
      <el-upload
        style="display: inline-block;"
        :show-file-list="false"
        ref="upload"
        :headers="header_token"
        :action="$network + '/emp/register/batch'"
        :limit="1"
        :file-list="fileList"
    ...
    <div class="content-container">
      ...
      <div class="content" style="margin-top: 10px;">
        ...
        >
          ...
          <el-table-column prop="registerUnit" label="注册单位"></el-table-column>
          <el-table-column prop="certName" label="证书名称"></el-table-column>
          <el-table-column prop="registerCardNo" label="注册证书编号"/>
          <el-table-column prop="registerNo" label="注册编号"></el-table-column>
          <el-table-column prop="majorOne" label="注册专业一">
            <template slot-scope="scope">
              <span
                v-for="item in dictMap.em_register_major"
                v-if="item.value == scope.row.majorOne"
              >{{item.label}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="majorTwo" label="注册专业二">
            <template slot-scope="scope">
              <span
                v-for="item in dictMap.em_register_major"
                v-if="item.value == scope.row.majorTwo"
              >{{item.label}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="issueDate" label="签发日期">
            <template slot-scope="scope">
              <span>{{ scope.row.issueDate == null ? '—' : $moment(scope.row.issueDate).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="validityDate" label="注册有效期">
            <template slot-scope="scope">
              <span>{{$moment(scope.row.validityDate).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  import { page, batchDel, exportExcel } from '@/api/employee/register'
  ...
  export default {
    ...
    mounted() {
      this.initPage()
      this.getDictMap('em_qualification,em_register_major,em_register_status')
    },
    methods: {
      // 批量导入成功
          ...
          if (this.selects.length > 0) {
            ...
            batchDel(ids).then(res => {
              if (res.code === 0) {
                ...
                this.$message({ message: '注册记录: ' + res.msg + '；记录所对应证书记录已通过审核，您已无权进行更新操作', type: 'warning' })
              } else {
                this.$message({ message: res.msg, type: 'warning' })
              }
            })
