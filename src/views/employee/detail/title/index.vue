  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-upload
        ...
        :action="$network + '/emp/title/batch'"
        :limit="1"
        :file-list="fileList"
    ...
    <div class="content-container">
      ...
      <div class="content" style="margin-top: 10px;">
        ...
        >
          ...
          <el-table-column prop="major" label="评定专业" />
          <el-table-column prop="certNo" label="证书编号" />
          <el-table-column prop="assessUnit" :show-overflow-tooltip="true" label="评定单位"></el-table-column>
          <el-table-column prop="assessTime" label="评定日期">
            <template slot-scope="scope">
              <span v-if="scope.row.assessTime">{{$moment(scope.row.assessTime).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="originalStorageLocation" label="原件存放位置" />
        </el-table>
      </div>
...
import { batchDel, page, exportExcel } from '@/api/employee/jobTitle'
        ...
        if (this.selects.length > 0) {
          ...
          batchDel(ids).then(res => {
            if (res.code === 0) {
              ...
              this.$message({ message: '职称记录: '+res.msg + "；记录所对应证书记录已通过审核，您已无权进行更新操作", type: 'warning' })
            } else {
              this.$message({ message: res.msg, type: 'warning' })
            }
          })
