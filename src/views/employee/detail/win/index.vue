    ...
    <div class="content-container">
      ...
      <div class="content" style="margin-top: 10px;">
        ...
        >
          ...
          <el-table-column prop="winLevel" label="获奖等级">
            <template slot-scope="scope">
              <span
                v-for="item in dictMap.em_win_level"
                v-if="item.value == scope.row.winLevel"
              >{{item.label}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="winDate" label="获奖日期">
            <template slot-scope="scope">
              <span>{{$moment(scope.row.winDate).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="certificateNo" :show-overflow-tooltip="true" label="证书编号"></el-table-column>
          <el-table-column prop="issuanceTime" label="发证日期">
            <template slot-scope="scope">
              <span v-if="scope.row.issuanceTime">{{$moment(scope.row.issuanceTime).format('YYYY-MM-DD')}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="grantUnit" label="发证机关/授予单位" />
        </el-table>
      </div>
...
import { batchDel, page, exportExcel } from '@/api/employee/winPrize'
