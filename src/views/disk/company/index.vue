  ...
  <div class="app-container">
    ...
    <div class="head-tools">
      ...
      <div class="file-create">
        ...
        <!-- 文件上传   TODO: headers 绑定 全局挂载的 mixins -->
        <el-upload
          ...
          <el-table-column prop="fileName" min-width="300" label="文件名称">
            <template slot-scope="scope">
              ...
              <span
                ...
                style="position: absolute; margin-top: 10px;cursor: pointer;"
...
export default {
  ...
  data () {
    return {
      ...
      // 分页查询参数
      pageParams: {
        bizId: getCompanyId(),        // 业务主键  当前为公司主键
        bizType: 1,                   // 业务类型   1: 公司文件  2: 项目文件  3: 个人文件
        keyword: null,                // 查询关键字
        pageSize: 10,                 // 每页个数
        pageNum: 0,                   // 当前页数
      ...
      treeParams: {                   // 树结构数据
        bizId: getCompanyId(),        // 公司ID  项目ID  员工ID
        bizType: 1,                   // 1:公司文件  2：项目文件  3：个人文件
        parentId: 0,                  // 最外层文件夹id
        isDir: 1                      // 是否为 文件夹树形列表 是：1 否：任意
      },
      // 文件路径 列表集合，默认根目录
      pathFiles: [{ id: 0, fileName: '企业文件' }],
  ...
  methods: {
    ...
    // 创建文件夹
    saveFolder () {
      let ids = [this.uploadParams.parentId]
      // 权限校验
      fileAuth(ids, this.power.folder).then(res => {
        if (res.code === 0) {
    ...
    deleteFile (id) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        // 权限校验
        fileAuth(ids, this.power.delete).then(res => {
        ...
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          fileAuth(ids, this.power.delete).then(res => {
            if (res.code === 0) {
              ...
              // 批量删除 如果返回无权限文件名称列表，提示具体文件 五权限操作
              if (res.data.length) {
                this.$message({ message: '您无 ' + res.data + ' 删除权限！', type: 'warning' })
              } else {
                this.$message({ message: res.msg, type: 'warning' })
    ...
    saveRename () {
      ...
      // 请求参数
      let ids = [this.rename.params.id]

      // 权限校验
      fileAuth(ids, this.power.rename).then(res => {
    ...
    moveProp (file) {
      ...
      // 权限校验
      fileAuth(ids, this.power.move).then(res => {
        if (res.code === 0) {
    ...
    handleRequest (data) {
      ...
      let ids = [this.uploadParams.parentId]
      // 权限校验
      fileAuth(ids, this.power.upload).then(res => {
        if (res.code === 0) {
