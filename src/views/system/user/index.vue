        ...
        <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
          ...
          <el-table-column label="部门 / 岗位">
            <template slot-scope="scope">
              <div>{{ scope.row.deptVo.name }} / {{ scope.row.jobVo.name }}</div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    edit(data) {
      ...
      _this.getDepts()
      _this.roleIds = []
      _this.form = { id: data.id, username: data.username, phone: data.phone, email: data.email, enabled: data.enabled.toString(), roles: [], deptVo: { id: data.deptVo.id }, jobVo: { id: data.jobVo.id }}
      ...
      _this.deptId = data.deptVo.id
      _this.jobId = data.jobVo.id
